...
# sales/views.py  ⟵ FILE INI
# --- Tambahkan import ini (kalau belum ada) ---
from geo.models import Location

# --- Tambahkan 3 helper di bawah import ---
...
def _origin_dest_types(transport_mode: str, service_option: str):
    ...
    # TRUCK / LAND → city <-> city
    if mode in ("TRUCK", "LAND", "ROAD"):
        return ({Location.CITY}, {Location.CITY})

    # AIR → 'port' = AIRPORT, SEA → 'port' = SEAPORT/JETTY
    if mode == "AIR":
    ...
    if opt in ("DOOR_TO_DOOR", "D2D"):
        return (city, city)
    if opt in ("DOOR_TO_PORT", "D2P"):
        return (city, port_types)
    if opt in ("PORT_TO_DOOR", "P2D"):
        return (port_types, city)
    if opt in ("PORT_TO_PORT", "P2P"):
        return (port_types, port_types)

    # Fallback: SEA/AIR → port-to-port
    return (port_types, port_types)
